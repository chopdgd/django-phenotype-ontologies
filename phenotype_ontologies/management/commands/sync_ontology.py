...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        logger.info('Ontology Version: {0}'.format(version))
        ...
        for term in data:
            ...
            try:
                term_obj, created = models.Term.objects.update_or_create(
            ...
            for key, value in term.relations.items():
                ...
                for related_term in value:
                    ...
                    if related_term.id.startswith('https://rarediseases.info.nih.gov/diseases/'):
                        logger.warning(
                            'Relationship {0} format does not conform!'.format(related_term.id))
                    else:
                        relations.append({
                            "type": type_obj,
                            "term": term_obj,
        ...
        for relation in relations:
            try:
                ...
                logger.warning(
                    'Relationship: {0} was not added. Error: {1}'.format(relation, error))
